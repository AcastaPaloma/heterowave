...
### Train FBPConvNet external comparison baseline

Implemented locally on 2026-06-21, not yet benchmarked on Drive.

Added model:

- `fbpconvnet`: classic sparse-view FBPConvNet-style residual image-domain
  U-Net. Input is only masked/sparse FBP, with no explicit coverage channels.
  This is intentionally simpler than the fair masked FBP U-Net.

Configs:

```text
configs/local_fbpconvnet_smoke.yaml
configs/colab_fbpconvnet.yaml
```

Train FBPConvNet on Colab:

```bash
python -m heterowave.train --config configs/colab_fbpconvnet.yaml
```
...
```bash
python -m heterowave.train \
  --config configs/colab_fbpconvnet.yaml \
  --resume /content/drive/MyDrive/heterowave/results/fbpconvnet_sparse/last.pt
```

Evaluate FBPConvNet validation:
...
python -m heterowave.evaluate \
  ...
  --heterowave-checkpoint /content/drive/MyDrive/heterowave/results/fbpconvnet_sparse/best.pt \
  --output-dir /content/drive/MyDrive/heterowave/results/fbpconvnet_sparse_validation
```
